...
then
    ...
    scripts/train.py --config=${config} --resume=${resume} --active_learning
else
echo "Single-GPU mode received..."
CUDA_VISIBLE_DEVICES=$devices python scripts/train.py --config=${config} --resume=${resume} --active_learning
fi
