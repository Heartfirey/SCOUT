...
class SINet_ResNet50(nn.Module):
    def __init__(self, config, channel=32):
        ...
        # Patch for ablation of the TalNet!
        if config.enable_ref_text:
            # channel down and up
            self.x0_down = nn.Conv2d(config.lateral_channels_in_collection[-1] + config.lateral_channels_in_collection[-2],
                                     config.img_channel_dim, 1, 1, 0)
            self.x2_down = nn.Conv2d(config.lateral_channels_in_collection[-3],
                                     config.img_channel_dim, 1, 1, 0)
            self.x0_up = nn.Conv2d(config.img_channel_dim, 
                                   config.lateral_channels_in_collection[-1] + config.lateral_channels_in_collection[-2], 1, 1, 0)
            self.x2_up = nn.Conv2d(config.img_channel_dim,
                                   config.lateral_channels_in_collection[-3], 1, 1, 0)
            # tfm module
    ...
    def forward(self, x, ref_text_dict=None):
        # ---- feature abstraction -----
        # - head
        ...
        # ---- Modify: Add text fusion --------
        if self.config.enable_ref_text:
            x0x1_down = self.x0_down(torch.cat((x0, x1), dim=1))
            x2_down = self.x2_down(x2)
            ref_text_dict = self.text_processor(ref_text_dict)
            x0x1_down, x2_down = self.fusion_layer([x0x1_down, x2_down], ref_text_dict)
            x01 = self.x0_up(x0x1_down)
            x2 = self.x2_up(x2_down)
        else:
