...
class SINet_v2(nn.Module):
    ...
    def __init__(self, config, channel=32, imagenet_pretrained=True):
        ...
        if self.config.enable_ref_text:
            self.x2_down = nn.Conv2d(512, self.config.img_channel_dim, 1, 1, 0)
            self.x3_down = nn.Conv2d(1024, self.config.img_channel_dim, 1, 1, 0)
            self.x4_down = nn.Conv2d(2048, self.config.img_channel_dim, 1, 1, 0)
            self.x2_up = nn.Conv2d(self.config.img_channel_dim, 512, 1, 1, 0)
            self.x3_up = nn.Conv2d(self.config.img_channel_dim, 1024, 1, 1, 0)
            self.x4_up = nn.Conv2d(self.config.img_channel_dim, 2048, 1, 1, 0)
    ...
    def forward(self, x, ref_text_dict=None):
        ...
        if self.config.enable_ref_text:
            ...
            x3_down = self.x3_down(x3)
            x4_down = self.x4_down(x4)
            
            ref_text_dict = self.text_processor(ref_text_dict)
            x2_down, x3_down, x4_down = self.fusion_layer([x2_down, x3_down, x4_down], ref_text_dict)
            
            x2 = self.x2_up(x2_down)
            x3 = self.x3_up(x3_down)
            x4 = self.x4_up(x4_down)
