...
class FSPNet(nn.Module):
    def __init__(self, config, ckpt="./weights/vit_base_patch16_384.pth", img_size=384):
        ...
        if self.config.enable_ref_text:
            self.channel_down = [
                nn.Conv2d(768, self.config.img_channel_dim, 1, bias=False) for i in range(12)
            ]
            
            self.channel_up = [
                nn.Conv2d(self.config.img_channel_dim, 768, 1, bias=False) for i in range(12)
            ]
    ...
    def text_fusion(self, img_feat, ref_text_dict):
        for i in range(12):
            print("[!]************************************************* -> FEAT-{}:   ".format(i), img_feat[i].shape)
        img_feat_down = [
            self.channel_down[i](img_feat[i]) for i in range(12)
        ]
        ref_text_dict = self.text_processor(ref_text_dict)
        fusion_feat = self.fusion_layer(img_feat_down, ref_text_dict)
        img_feat_up = [
            self.channel_up[i](fusion_feat[i]) for i in range(12)
        ]
        img_feat = [
            img_feat_up[i] + img_feat[i] for i in range(12)
        ]
        return img_feat

    def forward(self, img, ref_text_dict=None):
        # B Seq
        B, C, H, W = img.size()
        x = self.encoder(img)  # list 12x[8,576,768]
        if self.config.enable_ref_text:
            x = self.text_fusion(x, ref_text_dict)
